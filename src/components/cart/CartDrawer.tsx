...
export default function CartDrawer() {
  ...
  const deliveryFee = subtotal >= DELIVERY_CONFIG.freeDeliveryThreshold ? 0 : DELIVERY_CONFIG.baseFee
  const total = subtotal + deliveryFee
  const amountForFreeDelivery = DELIVERY_CONFIG.freeDeliveryThreshold - subtotal
  const freeDeliveryProgress = Math.min((subtotal / DELIVERY_CONFIG.freeDeliveryThreshold) * 100, 100)
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <button
                            onClick={() => updateQuantity(item.product.id, item.quantity + 1)}
                            className="w-7 h-7 rounded bg-primary-500 hover:bg-primary-600 text-white flex items-center justify-center transition-colors"
                          >
                ...
                {amountForFreeDelivery > 0 && (
                  <div className="bg-primary-50 p-3 rounded-xl">
                    <p className="text-sm text-primary-700 mb-2">
                      Add <span className="font-bold">{formatPrice(amountForFreeDelivery)}</span> more for FREE delivery!
                    </p>
